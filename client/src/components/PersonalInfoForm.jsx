import { Briefcase, BriefcaseBusiness, Globe, Icon, Linkedin, Mail, MapPin, Phone, User } from 'lucide-react'
    ...
    <div>
      ...
      <div className='flex items-center gap-2'>
        <label>
            ...
            <input type="file" name="" id="" accept='image/jpeg, image/png' className='hidden' onChange={(e)=>handleChange("image",e.target.files[0])}/>
        </label>
        {typeof data.image === 'object' && (
            <div className='flex flex-col gap-1 pl-4 text-sm'>
                <p>Remove Background</p>
                <label className='relative inline-flex items-center text-gray-900 gap-3'>
                    ...
                    <span className='absolute left-1 top-1 h-3 w-3 bg-white rounded-full transition-transform duration-200 ease-in-out peer-checked:translate-x-4'></span>
                </label>
            </div>
        )}
      </div>
      {fields.map((field)=>{
        ...
        return(
            <div key={field.key} className='space-y-1 mt-5'>
                ...
                <input type={field.type} value={data[field.key] || ""} onChange={(e)=>handleChange(field.key,e.target.value)} className='mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500 focus:border-blur-500 outline-none transition-colors text-sm' placeholder={`Enter your ${field.label.toLowerCase()}`} required={field.required}/>
            </div>
        )
      })}
    </div>
